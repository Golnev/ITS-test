...
def test_add_user_without_email_or_password(first_name: str,
                                            last_name: str,
                                            email: str,
                                            password: str,
                                            auth_headers):
    ...
    rs_api = request_utility.post(endpoint='users',
                                  payload=payload,
                                  headers=auth_headers,
                                  expected_status_code=400)

    assert rs_api['_message'] == 'User validation failed', (f'User added without mandatory data.'
                                                            f'Email: {email}, '
                                                            f'Password: {password}.')
...
def test_update_user_without_email_or_password(first_name: str,
                                               last_name: str,
                                               email: str,
                                               password: str,
                                               auth_headers):
